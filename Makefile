...
lint: ## check style with flake8 and pylint
	flake8 cert_hero tests
	pylint cert_hero tests

test: ## run unit tests quickly with the default Python
	pytest -v --cov=cert_hero --cov-report=term-missing tests/unit

test-all: ## run tests on every Python version with tox
	tox

coverage: ## check code coverage with unit tests quickly with the default Python
	coverage run --source cert_hero -m pytest tests/unit
	coverage report -m
	coverage html
	$(BROWSER) htmlcov/index.html

docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/cert_hero.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ cert_hero
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
